...
export async function courseRoutes(app) {
    ...
    app.get("/courses/:id", async (request, reply) => {
        ...
        if (!course)
            return reply.status(404).send({ message: "Curs inexistent" });
        return course;
    });
    // create course (professor only)
    app.post("/courses", { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    app.put("/courses/:id", { preHandler: [app.authenticate] }, async (request, reply) => {
        const { id } = request.params;
        const body = request.body;
        ...
        if (body?.title !== undefined) {
            ...
            if (!t)
                return reply.status(400).send({ message: "title invalid" });
            data.title = t;
        }
        if (body?.description !== undefined) {
            const d = body.description.trim();
            if (!d)
                return reply.status(400).send({ message: "description invalid" });
            data.description = d;
        }
        if (body?.category !== undefined) {
            const c = body.category.trim();
            if (!c)
                return reply.status(400).send({ message: "category invalid" });
            data.category = c;
        }
        return prisma.course.update({ where: { id }, data });
    });
    // delete course (owner only)
    app.delete("/courses/:id", { preHandler: [app.authenticate] }, async (request, reply) => {
        const { id } = request.params;
        const course = await prisma.course.findUnique({ where: { id } });
        if (!course)
            return reply.status(404).send({ message: "Curs inexistent" });
    ...
    app.post("/courses/:id/resources", { preHandler: [app.authenticate] }, async (request, reply) => {
        const { id } = request.params;
        const body = request.body;
        const course = await prisma.course.findUnique({ where: { id } });
        if (!course)
    ...
    app.delete("/resources/:id", { preHandler: [app.authenticate] }, async (request, reply) => {
        const { id } = request.params;
        const resource = await prisma.resource.findUnique({ where: { id } });
        if (!resource)
            return reply.status(404).send({ message: "Resursa inexistenta" });
    ...
    // add comment (auth)
    app.post("/courses/:id/comments", { preHandler: [app.authenticate] }, async (request, reply) => {
        const { id } = request.params;
        const body = request.body;
        const course = await prisma.course.findUnique({ where: { id } });
        if (!course)
        ...
        if (!content)
            return reply.status(400).send({ message: "content este obligatoriu" });
    ...
    app.delete("/comments/:id", { preHandler: [app.authenticate] }, async (request, reply) => {
        ...
        if (!comment)
            return reply.status(404).send({ message: "Comentariu inexistent" });
